const { formatLog, formatPath, readline } = require('../helpers'),
  rl = readline();

const verbose = (_verbose, display) => {
  _verbose &&
    rl.write(`\x1b[37mINFO> \x1b[33m${formatLog(display, 20)}`, 'verbose');
  return display;
};

const debug = (_debug, display) => {
  _debug &&
    rl.write(`\x1b[37mDEBUG> \x1b[31m${formatLog(display, 40)}`, 'debug');
  return display;
};
...
const loadingBar = () => {
  ...
  return {
    ...
    showBar: () => {
      ...
      const title = !isDone
        ? formatPath(taskName, 90)
        : formatPath(this.lastTaskName, 90);
      const bar = '='.repeat(done) + '-'.repeat(total);
      const info = formatPath(
        this.taskDone.toString(),
        this.totalTask.toString().length
      );
      const percentage = formatPath(
